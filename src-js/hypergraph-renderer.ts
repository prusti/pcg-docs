import cytoscape, { Core, EdgeDefinition, NodeDefinition, ElementDefinition } from 'cytoscape';
import Layers from 'cytoscape-layers';
import BubbleSets from 'cytoscape-bubblesets';
import { COUPLING_ALGORITHMS, applyCouplingAlgorithm } from './coupling-algorithms';
...
(function() {
    ...
    function renderAllHypergraphs(): void {
        ...
        containers.forEach(function(container) {
            ...
            try {
                const graphData = JSON.parse(dataElement.textContent || '{}') as GraphData;
                renderHypergraph(container as HTMLElement, graphData);
                container.setAttribute('data-rendered', 'true');
            } catch (e) {
                console.error('Failed to render hypergraph:', e);
                container.innerHTML = '<div class="hypergraph-error">Failed to render graph: ' + (e as Error).message + '</div>';
            }
        });
    }
    ...
    function renderHypergraph(container: HTMLElement, data: GraphData): void {
        ...
        function renderGraph(couplingAlgorithmId: string): RenderResult {
            ...
            if (couplingAlgorithmId && couplingAlgorithmId !== 'none' && data.edges && data.nodes) {
                try {
                    const coupledEdges = applyCouplingAlgorithm(couplingAlgorithmId, data.nodes, data.edges);
                    ...
                    edgesToRender = [...edgesToRender, ...coupledEdgeObjects];

                    const infoSpan = document.getElementById(`coupling-info-${container.id}`);
                    if (infoSpan) {
            ...
            edgesToRender.forEach(function(edge, idx) {
            ...
            return { elements, hyperedgeGroups };
        }
        ...
        if (couplingSelector) {
            const select = document.getElementById(`coupling-select-${container.id}`) as HTMLSelectElement;
            if (select) {
