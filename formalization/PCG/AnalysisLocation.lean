...
defStruct AnalysisLocation (.doc (.plain "al"),
  ...
  "Analysis Locations"
  (.plain "An analysis location identifies a program point \
   at which the PCG state is observed: a MIR location paired \
   with the evaluation phase of the statement at that location.")
where
