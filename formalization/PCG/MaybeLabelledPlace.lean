...
defEnum MaybeLabelledPlace {P}
  ...
  "A maybe-labelled place is either a current place or a \
   place labelled with a snapshot location. The place set \
   P is a parameter of the definition (similar to the base \
   and index sets of a LifetimeProjection), so the same \
   constructor can be reused with different underlying \
